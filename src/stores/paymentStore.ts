...
export const usePaymentStore = create<PaymentState>((set) => ({
  ...
  fetchChannels: async () => {
    ...
    if (response.success && response.data) {
      ...
      set({
        error: response.error || "Failed to fetch payment channels",
        isLoading: false,
      });
    }
  },
  ...
  fetchChannel: async (code: string) => {
    ...
    if (response.success && response.data) {
      ...
      set({
        error: response.error || "Failed to fetch payment channel",
        isLoading: false,
      });
    }
  },

  calculateFee: async (amount: number, code: string) => {
    const response = await api.calculateFee(amount, code);
